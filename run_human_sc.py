...
q = 0.2
nrep = 2
l1 = 0.00001 # l1 regularization factor in mlp
lr = 0.0005 # learning rate for mlp training
...
for i in range(nrep):
    ...
    dp.fit(Xnew_train, y_train, epochs=dnn_epoch, batch_size=bs, verbose=dnn_verb, validation_split=0.1, callbacks=[es])
    ...
    pig.fit(Xnew_train, y_train, max_epoch=dnn_epoch,lr=lr,l1=l1,patience=10,y_design='clf',es='./checkpoint/'+pig_name+"_"+str(i)+'.pt')
    ...
    if len(selected_pink) == 0:
        ...
        mrefit.fit(X1[ind_train,:][:,selected_pink], y[ind_train], epochs=200, batch_size=bs, verbose=dnn_verb)
    ...
    if len(selected_pig) == 0:
        ...
        mrefit_pig.fit(X1[ind_train,:][:,selected_pig], y[ind_train], epochs=200, batch_size=bs, verbose=dnn_verb)
    ...
    mrefit_all.fit(X1[ind_train,:], y[ind_train], epochs=300, batch_size=bs, verbose=dnn_verb)
