...
os.chdir('./real_data/human_microbiome')

ds = 'microbiome_data_common'
d = 30
...
for i in range(nrep):
    ...
    pink = Sequential()
    pink.add(PairwiseConnected(input_shape=(2 * d,)))
    pink.add(Dense(d, activation=dnn_met,
                    kernel_regularizer=keras.regularizers.l1_l2(l1=0.001, l2=0.001)))
    pink.add(Dense(1, activation='elu',
                    kernel_regularizer=keras.regularizers.l1_l2(l1=0.001, l2=0.001)))
    pink.compile(loss=dnn_loss, optimizer=keras.optimizers.Adam())
    pink.fit(Xnew_train, y_train, epochs=dnn_epoch, batch_size=bs, verbose=dnn_verb)


    weights = pink.get_weights()
    ...
    print("DeepPINK selected :",selected_pink)
    result_pink.iloc[selected_pink] += 1
    mat_selected_pink[i,selected_pink] = 1
    sel_count_pink.append(len(selected_pink)) 


    yhat_train_pink[i,:] = [1 if a > 0.5 else 0 for a in pink.predict(Xnew_train).flatten()]
    pe_train_pink[i] = np.mean(yhat_train_pink[i,:] != y_train.flatten())


    #### DeepPIG ####
    pig = DeepPIG(Xnew.shape[1], print_interval=100).to(device)
    pig.fit(Xnew_train, y_train, max_epoch=dnn_epoch,lr=0.001,l1=l1,y_design='clf', patience=100,es='./checkpoint/'+pig_name+"_"+str(i)+'.pt')
    selected_pig = pig.knockoff_select(ko_plus=False,q=q)


    print("DeepPIG selected :",selected_pig)
    ...
    ## refit to calculate PE (DeepPINK) ##
    if len(selected_pink) == 0:
        ...
        mrefit = Sequential()
        es = EarlyStopping(monitor='val_loss', patience=30, verbose=2)
        mrefit.add(Dense(s, input_dim=s, activation='relu',kernel_initializer=initializer))
        mrefit.add(Dense(1, activation='sigmoid',kernel_initializer=initializer))
    ...
    if len(selected_pig) == 0:
        ...
        es = EarlyStopping(monitor='val_loss', patience=30, verbose=2)
        mrefit_pig.add(Dense(s, input_dim=s, activation='relu',kernel_initializer=initializer))
        mrefit_pig.add(Dense(1, activation='sigmoid',kernel_initializer=initializer))
        mrefit_pig.compile(loss=dnn_loss, optimizer=keras.optimizers.Adam())
    ...
    mrefit_all.compile(loss=dnn_loss, optimizer=keras.optimizers.Adam())
